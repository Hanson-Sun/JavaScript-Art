...
function setup() {
	...
	boidlist = [];
	s = random(0.5,1)
	for (let i = 0; i < (100+100/s); i++) {
		boidlist.push(new Boid(new Vector2D(Math.random() * 600, Math.random() * 600), s, random(50,70)));
	}
...
function draw() {
	...
	for (let i = 0; i < boidlist.length; i++) {
		...
		boid.steer(boidlist);
		boid.avoid(boidlist);
		//boid.turnTo(new Vector2D(mouseX, mouseY));
		boid.step(2);
		boid.draw();
	}
