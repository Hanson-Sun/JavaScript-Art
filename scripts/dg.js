...
function setup() {
	...
	for (i = 0; i < 1600; i++) {
		new Circle(cl, new Vector2D(random(width),random(height)), new Vector2D(0, 0), random(0.5, 1), "rgba(255,255,255, 0.1)", false);
	}
	weighted = -0.8
...
function draw() {
	...
	// }
	if (cl.length < 2000) {
		new Circle(cl, checkgenerate(), new Vector2D(0, 0), random(0.5, 1), "rgba(255,255,255, 0.1)", false);
	}
